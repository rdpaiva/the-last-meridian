...
├── WingCommander (player-side wing doctrine: leader failover on player death + carrier scramble)
...
shared/src/                @space-duel/shared — the SIM + config + AI: everything a headless
  ...
  FleetCommander.ts        enemy fleet doctrine (strikers/escorts/dynamic pool) re-tasked ~2s via AIController.setOrder(); keeps world.leader on a LIVE striker (failover)
  WingCommander.ts         PLAYER-side wing doctrine (FleetCommander's sibling, same commander.* knobs, zero RNG): leader failover on player death (senior escort spearheads, wing packs on it) + carrier-defense scramble
  ...
  Difficulty.ts            the DIFFICULTY CATALOG (easy/normal/hard AI-skill presets): applyDifficulty writes ai.*/commander.* knobs into GameConfig pre-construction (injectable override hook, Maps pattern) — shared so BattleRoom applies the room creator's pick server-side (where the AI pilots live); replicated as BattleState.difficulty
...
client/src/                @space-duel/client — the Babylon view, menus, and entry (Vite root)
  ...
  game/
    ...
    LoadoutMenu.ts         the splash front door, THREE STEPS in a fixed header-rail/stage/footer-rail frame: 1 MODE (solo/multiplayer + callsign; returning players get a gold CONTINUE CTA) → 2 HANGAR (faction cards → roster + live preview) → 3 MISSION (solo: difficulty+arena+deployment FLY/OBSERVE — OBSERVE launches the AI-vs-AI exhibition, per-launch, never persisted; online: quick-match/invite briefing + difficulty+arena pickers for the room creator, invite joins inherit) → LAUNCH fires onPlay(mode). MODE→HANGAR advance first offers main.ts the firstRunIntro gate. Keyboard-driven; owns ENTER in factionSelect (yields to the controls overlay and the Field Manual while they're open). Footer links = controls overlay + LoadoutActions (field manual / replay intro / match settings)
    FieldManual.ts         the "hit the ground running" guide: a self-paced card deck (one gameplay concept per card — flight, weapons, carrier ops + Meridian Drive, ship roles, terrain, stations/Energy/shields, HUD/sensors) opened from the loadout footer link or the gold ROOKIE PILOTS callout (shared with Flight School; retires after both onboarding paths are complete). ALL text lives in buildCards() — edit/add lines there; timing numbers interpolate live from GameConfig. Visuals are game-rendered (ShipPreview thumbnails, HUD-color specimens, inline SVG) — no art assets
    FlightSchool.ts        interactive five-lesson training director (flight → guns → missile lock → carrier service → jump): observes the REAL solo Ship + SimEventBus, stages one inert/invulnerable target drone, depletes the ship for the service exercise, and persists graduation. FlightSchoolProgress.ts is the pure/tested lesson state machine
    ...
    Difficulty.ts          client shim over the SHARED difficulty catalog (shared/Difficulty.ts): selection persistence (lastMeridian_difficulty) + the SOLO applyDifficulty that wires ConfigOverrides precedence in. Online the ROOM owns the difficulty: the creator's pick rides JoinOptions, BattleRoom applies it server-side; player's own wing unaffected either way
    ...
    MapEditor.ts           splash map-editor screen (data-state="mapEditor", admin AUTHORING tooling): top-down 2D canvas + brush palette paints a MapConfig (nebula/storm/rock-field/wreck circles, capture stations, draggable carriers; brushes are STICKY — editing a shape re-seeds its brush for the next stamp); COPY MAP emits a paste-ready MAPS entry for shared/Maps.ts (maps stay compile-time presets); TEST FLIGHT solo-launches the draft via shared applyMapConfig; draft persists (lastMeridian_mapDraft)
