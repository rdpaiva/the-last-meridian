...
client/src/                @space-duel/client — the Babylon view, menus, and entry (Vite root)
  ...
  game/
    ...
    LoadoutMenu.ts         the splash front door, THREE STEPS in a fixed header-rail/stage/footer-rail frame: 1 MODE (solo/multiplayer + callsign; returning players get a gold CONTINUE CTA) → 2 HANGAR (faction cards → roster + live preview) → 3 MISSION (solo: difficulty+arena+deployment FLY/OBSERVE — OBSERVE launches the AI-vs-AI exhibition, per-launch, never persisted; online: quick-match/invite briefing) → LAUNCH fires onPlay(mode). MODE→HANGAR advance first offers main.ts the firstRunIntro gate. Keyboard-driven; owns ENTER in factionSelect (yields to the controls overlay and the Field Manual while they're open). Footer links = controls overlay + LoadoutActions (field manual / replay intro / match settings)
