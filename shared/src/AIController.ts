...
export type AIOrder =
  ...
  | "defend"
  /** Dock at the assigned capture station (setCaptureTarget) until it flips:
   *  fly there, coast below the dock speed inside the radius, break to engage
   *  enemies near the STATION, resume. No target set = patrol. */
  | "capture";
...
export class AIController implements ShipController {
  ...
  /** The station a "capture" order works (assigned via setCaptureTarget). */
  private captureTarget: CaptureStation | null = null;
  /**
  ...
  /**
   * Assign the station a "capture" order works (FleetCommander calls this
   * alongside setOrder("capture")). Kept separate from setOrder so the order
   * signature stays stable; a capture order with no target patrols.
   */
  setCaptureTarget(station: CaptureStation | null): void {
    this.captureTarget = station;
  }
  ...
  update(deltaSeconds: number, self: Ship, world: ControllerWorld): InputState {
    ...
    const perFrame =
      ...
      this.order === "defend" || this.order === "capture";
    ...
    switch (this.order) {
      ...
      case "capture": {
        const station = this.captureTarget;
        if (!station) {
        ...
        // Guard the capture bubble: engage anything near the STATION (like
        // "defend" guards the carrier), then come back and finish the flip.
        const intruder = this.nearestLiveOpponentToPoint(
          world, sx, sz, cfg.captureEngageRange,
        );
        if (intruder) {
          steerHeading = this.headingTo(self, intruder.position.x, intruder.position.z);
          aim = intruder;
        ...
        const dz = sz - self.position.z;
        const dist = Math.hypot(dx, dz);
        if (dist > station.radius * 0.5) {
          // Approach; cut thrust once inside the radius so momentum carries
          // us to the middle instead of blowing through the far side.
          steerHeading = this.headingTo(self, sx, sz);
          thrust = dist > station.radius;
        } else {
          // Docked: hold heading and bleed speed below the dock gate (drag
          // does most of it; a dab of reverse thrust brakes a hot entry).
          steerHeading = self.rotationY;
          thrust = false;
          reverse = self.speed > GameConfig.stations.dockMaxSpeed * 0.8;
        }
        break;
      }
