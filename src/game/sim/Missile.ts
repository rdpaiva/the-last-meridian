...
/**
 * Single heat-seeking missile — SIM state only since the Missile/MissileSystem
 * split (docs/MULTIPLAYER.md Phase 0): position, heading, homing steering,
 * age, kill flag. Owned and updated by MissileSystem; its composite mesh +
 * exhaust trail live in MissileSystemView (src/game/view/), which tracks live
 * rounds each frame. Do not construct directly from outside MissileSystem.
 *
...
export class Missile implements Interceptable {
  ...
  constructor(
    /** World-space position (starts at the launch point ahead of the nose). */
   ...
   * MissileSystem sweep will see isExpired === true and drop it (the view
   * disposes its mesh + trail when the round disappears).
   */
  kill(): void {
